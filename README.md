...
- 定向代理：每个账号可指定走不同的代理节点（VPN），未指定的走系统默认网络
...
分组允许为空：账号可以不属于任何分组（显示为“未分组”）。删除分组只会把组内账号变回未分组，**不会删除账号**。

## 定向代理（每账号走不同节点）

在“代理节点”页填入 Clash 订阅地址点“导入”，随后在账号行的下拉里为每个账号选择节点。

- 未选节点的账号走**系统默认网络**（也就是你 Clash 当前选的节点）
- 例：本机 Clash 选日本，账号 A 绑美国节点、账号 B 绑韩国节点、账号 C 不绑 → A 走美国、B 走韩国、C 走日本
- 订阅**只在你点“导入”或“手动刷新订阅”时才会更新**，不做任何自动刷新

实现方式：程序另起一个私有 mihomo 进程，为每个启用节点开一个本地端口（21000 起），
...
若账号绑定的节点已停用或已不在订阅中，该账号会直接拒绝启动并提示，而不是悄悄用错误的出口 IP 访问。
...
| 需重新登录 | cookie 还在、看着像已登录，但令牌已被服务端作废（常见于在别处改了密码或新增双重认证）。点该行的“重新登录”会先清掉失效会话再打开登录页，**不必删除账号**，备注/分组/代理配置都会保留 |
