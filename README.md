...
- 启动时后台和界面共用串行的 Agent 连接流程。首次连接超时后按 1、2、5、10、20、30 秒间隔最多重试六次，仍在启动的 Agent 不会因重试而被回收。侧栏离线时的刷新按钮可手动重新连接，无需退出全部。
- 绑定节点的账号通过 `Chrome → 本机私有 mihomo → 分组节点` 访问网络；未绑定节点的账号跟随系统网络。分组代理独立于系统 HTTP 代理设置，但仍受操作系统路由、TUN/VPN 和 DNS 接管影响。
- 如果系统节点失效会连带影响分组节点，先关闭系统代理软件的 TUN／虚拟网卡模式进行对照（仅关闭“系统代理”开关不够）。需要保留 TUN 时，可尝试在系统代理的规则模式中按代理内核的完整进程路径配置 `DIRECT`，并验证规则命中和实际出口；这不是应用已经验证的自动修复。如果工具复用 Clash Verge 的可执行文件，路径规则会匹配该路径下的所有内核实例，不能只区分工具的私有进程。
- TUN 全局路由和路由排除的含义见 [mihomo 官方文档](https://wiki.metacubex.one/en/config/inbound/tun/)。应用不会自动修改其他代理软件或系统路由。
...
8. 分别执行 Authenticode、Apple Developer ID/公证/stapling、Linux Minisign 门禁；
   `v0.2.10` 有一个写死版本号、不可复用的一次性 unsigned 发布例外。
...
缺少任一平台凭据时，工作流仍会生成带 `UNSIGNED-<rid>.txt` 标记的内部检查产物，但正常版本拒绝创建 GitHub Draft。正式发布要求 Windows Authenticode、macOS Developer ID + Apple 公证以及 Linux Minisign 全部通过。经本次明确授权，`v0.2.10` 可使用专用开关跳过这些平台签名与安装更新验收并直接公开；Tauri updater 私钥、公钥、四平台构建、`.sig`、SBOM 与校验和仍是硬门禁，该开关不能用于任何其他版本。具体见 [远端发布流程](docs/RELEASE_REMOTE.md)。
...
`-Mode PublishDraft -UpdaterVerified`，版本才会进入客户端的稳定更新通道。`v0.2.10`
