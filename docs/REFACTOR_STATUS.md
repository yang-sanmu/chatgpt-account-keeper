...
| ~~No browser/WebView management panel~~ → Management UI loads no remote content | REVISED | **Reversed by the Tauri migration** (`docs/TAURI_MIGRATION_PLAN.md`). The original requirement banned WebView outright. The replacement is three verifiable constraints: the release bundle listens on no port, its CSP admits no remote origin, and the management UI never reaches chatgpt.com — that is the real Chrome's job. The Avalonia client still satisfies the stricter original form; the release verifier still rejects the legacy web panel. |
| No management `localhost` listener | DONE | Installed Agent uses Named Pipe/Unix socket. Express is a development-only legacy adapter and is excluded from releases. Tauri serves its frontend from the binary over a custom protocol, not a TCP server. |
...
| Draft GitHub Release workflow/SBOM/licenses | DONE | The workflow, package verifier, private-runtime licenses and CycloneDX/SPDX SBOM generation are implemented. Normal releases remain gated by platform signing and an attested N-1 → N run; exact `v0.2.10` has a one-time direct-public exception. |
...
Exact `v0.2.2` received the same separately scoped authorization on 2026-09-03;
Exact `v0.2.3` received a new separately scoped authorization on 2026-09-03;
Exact `v0.2.4` received a new separately scoped authorization on 2026-09-11;
none turns repository credentials or real-machine acceptance into
