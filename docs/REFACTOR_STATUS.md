...
| Draft GitHub Release workflow/SBOM/licenses | DONE | The workflow, package verifier, private-runtime licenses and CycloneDX/SPDX SBOM generation are implemented. Normal releases remain gated by platform signing and an attested N-1 → N run; exact `v0.2.8` has a one-time direct-public exception. |
