...
## `v0.2.8` 一次性 unsigned 发布例外

经发布者针对本次发布明确授权，工作流允许精确版本 `0.2.8` 在缺少 Authenticode、Apple
Developer ID/公证和 Linux Minisign 时直接公开，并跳过候选安装、N → N+1 与
`PublishDraft -UpdaterVerified` 阶段：

~~~powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.8 -Mode Release -PublishUnsignedV028 -ReleaseNotesFile .\docs\release-notes\v0.2.8.md
~~~

这个命令只跟踪远端工作流，不下载任何产物。CI 会同时要求版本精确等于 `0.2.8`、四个平台
...
`v0.2.8` 一次性例外没有 Minisign `.minisig` 与 `minisign.pub`，但仍包含 Linux SHA-256
...
除写死为 `v0.2.8` 的本次发布开关外，不保留“允许无平台签名并跳过人工验收”的直接
公开路径。
