...
## `v0.2.10` 一次性 unsigned 发布例外

经发布者针对本次发布明确授权，工作流允许精确版本 `0.2.10` 在缺少 Authenticode、Apple
Developer ID/公证和 Linux Minisign 时直接公开，并跳过候选安装、N → N+1 与
`PublishDraft -UpdaterVerified` 阶段：

~~~powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.10 -Mode Release -PublishUnsignedV0210 -ReleaseNotesFile .\docs\release-notes\v0.2.10.md
~~~

这个命令只跟踪远端工作流，不下载任何产物。CI 会同时要求版本精确等于 `0.2.10`、四个平台
...
这只是一次发布决策，不代表平台签名、真机安装或 updater 更新链路已经验收。后续版本仍走
下面的正常 Candidate → Draft → N → N+1 → PublishDraft 流程。
...
node scripts/verify-release-version.mjs 0.2.5
git add -A
...
~~~powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.5 -Mode Candidate -ReleaseNotesFile .\release-notes-0.2.5.md
~~~

成功后，脚本把聚合 artifact 下载到 artifacts\candidate-0.2.5。Candidate 不创建 tag，
...
~~~powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.5 -Mode Release -NMinusOneVerified -ReleaseNotesFile .\release-notes-0.2.5.md
~~~
...
~~~powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.5 -Mode PublishDraft -UpdaterVerified
~~~
...
`v0.2.10` 一次性例外没有 Minisign `.minisig` 与 `minisign.pub`，但仍包含 Linux SHA-256
...
除写死为 `v0.2.10` 的本次发布开关外，不保留“允许无平台签名并跳过人工验收”的直接
公开路径。
