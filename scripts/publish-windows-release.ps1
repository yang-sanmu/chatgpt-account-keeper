...
param(
    ...
    # One-time authorization for v0.2.2. This publishes directly while retaining
    # updater signatures and all build and asset-integrity gates.
    [switch] $PublishUnsignedV022,
...
function Invoke-ReleaseWorkflow {
    ...
    $arguments = @(
        ...
        '--raw-field', "publish_unsigned_v0_2_2=$($PublishUnsignedV022.ToString().ToLowerInvariant())"
    )
...
if ($PublishUnsignedV022 -and $Mode -ne 'Release') {
    throw '-PublishUnsignedV022 is only valid with -Mode Release.'
}
if ($PublishUnsignedV022 -and $Version -ne '0.2.2') {
    throw '-PublishUnsignedV022 is permanently restricted to version 0.2.2.'
}
if ($PublishUnsignedV022 -and $NMinusOneVerified) {
    throw '-PublishUnsignedV022 cannot be combined with -NMinusOneVerified.'
}
if ($Mode -in $attestingModes -and -not $NMinusOneVerified -and -not $PublishUnsignedV022) {
...
try {
    ...
    $runId = Invoke-ReleaseWorkflow `
        -GitHubCli $githubCli `
        -PublishDraft (-not $PublishUnsignedV022.IsPresent) `
        -Verified $NMinusOneVerified.IsPresent `
        -PublishUnsignedV022 $PublishUnsignedV022.IsPresent
    if ($null -ne $runId) {
        if ($PublishUnsignedV022) {
