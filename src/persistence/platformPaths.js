...
function requiredHome(homeDir) {
  const value = path.resolve(String(homeDir || ""));
  if (!homeDir || value === path.parse(value).root) {
...
export function resolvePlatformPaths({
  ...
  const home = requiredHome(homeDir);
  let defaultDataRoot;
  let bootstrapFile;
  let cacheRoot;
  let runtimeRoot;
  ...
  if (platform === "win32") {
    const local = path.resolve(env.LOCALAPPDATA || path.join(home, "AppData", "Local"));
    const roaming = path.resolve(env.APPDATA || path.join(home, "AppData", "Roaming"));
    defaultDataRoot = path.join(local, APP_DIRECTORY_NAME, "data");
    bootstrapFile = path.join(roaming, APP_DIRECTORY_NAME, "bootstrap.json");
    cacheRoot = path.join(local, APP_DIRECTORY_NAME, "cache");
    runtimeRoot = path.join(local, APP_DIRECTORY_NAME, "run");
    stateRoot = path.join(local, APP_DIRECTORY_NAME, "state");
  } else if (platform === "darwin") {
    const support = path.join(home, "Library", "Application Support", APP_DIRECTORY_NAME);
    defaultDataRoot = support;
    bootstrapFile = path.join(support, "bootstrap.json");
    cacheRoot = path.join(home, "Library", "Caches", APP_DIRECTORY_NAME);
    runtimeRoot = path.join(cacheRoot, "run");
    stateRoot = path.join(support, "state");
  } else {
    const xdgData = path.resolve(env.XDG_DATA_HOME || path.join(home, ".local", "share"));
    const xdgConfig = path.resolve(env.XDG_CONFIG_HOME || path.join(home, ".config"));
    const xdgCache = path.resolve(env.XDG_CACHE_HOME || path.join(home, ".cache"));
    const xdgState = path.resolve(env.XDG_STATE_HOME || path.join(home, ".local", "state"));
    defaultDataRoot = path.join(xdgData, "gpt-account-keeper");
    bootstrapFile = path.join(xdgConfig, "gpt-account-keeper", "bootstrap.json");
    cacheRoot = path.join(xdgCache, "gpt-account-keeper");
    stateRoot = path.join(xdgState, "gpt-account-keeper");
    runtimeRoot = env.XDG_RUNTIME_DIR
      ? path.join(path.resolve(env.XDG_RUNTIME_DIR), "gpt-account-keeper")
      : path.join(cacheRoot, "run");
  }

  const resolvedDataRoot = dataRoot ? path.resolve(dataRoot) : defaultDataRoot;
  return Object.freeze({
    dataRoot: resolvedDataRoot,
    databaseFile: path.join(resolvedDataRoot, "keeper.db"),
    profilesRoot: path.join(resolvedDataRoot, "profiles"),
    profilesArchiveRoot: path.join(resolvedDataRoot, "profiles-archive"),
    profileTrashRoot: path.join(resolvedDataRoot, "profile-trash"),
    backupsRoot: path.join(resolvedDataRoot, "backups"),
    migrationRoot: path.join(resolvedDataRoot, "migration"),
    // 单实例锁跟着数据目录走：同一份数据库和 Profile 只允许一个 Agent 写。
    instanceLockFile: path.join(resolvedDataRoot, "agent.lock"),
    bootstrapFile,
    cacheRoot,
    runtimeRoot,
    stateRoot,
...
function existingRealPath(value, fsImpl) {
...
export function validateDataRoot(
  ...
  if (!path.isAbsolute(trimmed)) throw new Error("数据目录必须是绝对路径");
  if (platform === "win32" && /^(?:\\\\|\/\/)/.test(trimmed)) {
    throw new Error("数据目录不能位于 UNC/网络共享");
  }

  const resolved = existingRealPath(trimmed, fsImpl);
  if (resolved === path.parse(resolved).root) throw new Error("数据目录不能是文件系统根目录");
  ...
  for (const [label, forbidden] of [
    ...
    const actualForbidden = existingRealPath(forbidden, fsImpl);
    if (
